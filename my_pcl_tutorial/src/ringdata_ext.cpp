...
void input(const sensor_msgs::PointCloud2ConstPtr &msg1, const sensor_msgs::PointCloud2ConstPtr &msg2)
{
    ...
    *merged_cloud = *cloud1 + *cloud2;
    ...
    // Extract points with the desired ring number
    pcl::PointCloud<PointXYZIRT>::Ptr filtered_cloud(new pcl::PointCloud<PointXYZIRT>());
    uint16_t target_ring = 3; // Change this to the desired ring number
    for (size_t i = 0; i < obstacle_cloud->size(); ++i)
    {
        if (obstacle_cloud->points[i].ring == target_ring)
        {
            filtered_cloud->push_back(obstacle_cloud->points[i]);
        }
    }
